...
def replace_blank_space(file_path:Path, replacement = "_"):
        file_name:str = file_path.name
        new_file_name = file_name.replace(" ", replacement)
...
parser = argparse.ArgumentParser(
    description="""     Short script for renaming mrc files that have blankspace in their names.
                        The blankspace is replaced by an underscore.
                        Relion CTFfind4 and several other programs don't like filenames with blankspace"""
    )
...
for img_file_p in img_dir_p.glob(args.glob_pattern):
    print(f"Replacing blank space in: {img_file_p}" )
